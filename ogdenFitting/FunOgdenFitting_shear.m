...
function f=FunOgdenFitting_shear(x)
mu=[x(1),x(2),x(3)];
a=[x(4),x(5),x(6)];
testData=csvread('rubberTestShear.csv',2,0);
e_shear_test=testData(:,1);
sigma_shear_test=testData(:,2);
...
for (i=1:1:N)
    sigma_shear_calc=sigma_shear_calc+mu(i)*(e_shear_test.^(a(i)-1)-e_shear_test.^(-1*a(i)-1));    
end
% figure;
% plot(e_shear_test,sigma_shear_test,'r');
% hold on;
% plot(e_shear_test,sigma_shear_calc);

f=sum((sigma_shear_calc-sigma_shear_test).^2);
%  plot_shear([0.00832,-0.035427,0.06827,3.8027,2.4725,0.8653])
