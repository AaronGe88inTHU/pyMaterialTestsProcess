...
def readTestInfo(fileMachine, fileDic, direction ='u_c',fileCamera ='0_45mmpermin-5.csv'):
    """
    return
        time
        result
    ...
    cameraTime = camera.loc[1:, 2].values.reshape(-1,1).astype(float)
    #print(cameraTime)
    dic = pd.read_csv(fileDic)
    result = dic[direction].values.reshape(-1,1).astype(float)
    count = result.shape[0]
    
    cameraTime = cameraTime[0:count] - cameraTime[0]
    #print (cameraTime)
